 ...
 */
  ...
  Future<int> listarBienesAllPorCiudad() async {
    ...
    for (int i = 0; i < decodedData["bienes"].length; i++) {
      //-----ingresamos Good
      BienesModel goodmodel = BienesModel();
      goodmodel.idGood = decodedData["bienes"][i]['id_good'];
      goodmodel.goodName = decodedData["bienes"][i]['good_name'];
      goodmodel.goodSynonyms = decodedData["bienes"][i]['good_synonyms'];
      await goodDatabase.insertarGood(goodmodel);
  ...
  Future<dynamic> obtenerBienesPorIdItemsubcategoria(String id) async {
    try {
      var response = await http.post(
          "$apiBaseURL/api/Inicio/listar_bienes_por_id_itemsubca",
          body: {'id_ciudad': '1', 'id_itemsubcategoria': '$id','app':'true'}
      );
      ...
      for (int i = 0; i < decodedData["bienes"].length; i++) {
        ...
        subsidiaryGoodModel.productoMeasure =
            listBienes['subsidiary_good_stock_measure'];
        subsidiaryGoodModel.productoRating =
            listBienes['subsidiary_good_rating'];
        subsidiaryGoodModel.productoUpdated =
            listBienes['subsidiary_good_updated'];
        subsidiaryGoodModel.productoStatus =
            listBienes['subsidiary_good_status'];

        var productList =
            await productoDatabase.obtenerProductoPorIdSubsidiaryGood(
                listBienes['id_subsidiarygood']);
        ...
        if (list.length > 0) {
          subsidiaryModel.idSubsidiary = listBienes['id_subsidiary'];
          ...
          subsidiaryModel.subsidiaryCellphone =
              listBienes['subsidiary_cellphone'];
          subsidiaryModel.subsidiaryCellphone2 =
              listBienes['subsidiary_cellphone_2'];
          ...
          subsidiaryModel.subsidiaryOpeningHours =
              listBienes['subsidiary_opening_hours'];
          subsidiaryModel.subsidiaryPrincipal =
              listBienes['subsidiary_principal'];
