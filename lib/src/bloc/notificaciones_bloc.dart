...
class NotificacionesBloc {
  ...
  final listarNotificacionesController = BehaviorSubject<List<NotificacionesModel>>();
  final notificacionesPendientesController = BehaviorSubject<List<NotificacionesModel>>();

  Stream<List<NotificacionesModel>> get listarnotificacionesStream => listarNotificacionesController.stream;

  Stream<List<NotificacionesModel>> get notificacionesPendientesStream => notificacionesPendientesController.stream;
...
// notificaciones: 0:No leídas, 1:leídas
  void listarNotificaciones() async {
    listarNotificacionesController.sink.add(await notificacionDb.obtenerNotificaciones());
...
// notificaciones pendientes
  void listarNotificacionesPendientes() async {
    notificacionesPendientesController.sink.add(await notificacionesPendientes());
  }
