...
class SugerenciaBusquedaDb {
  ...
  insertarProducto(SugerenciaBusquedaModel sugerenciaBusqModel) async {
    try {
      ...
      final res = await db.rawInsert("INSERT OR REPLACE INTO SugerenciaBusqueda (id_itemsubcategory,nombre_producto,id_producto,tipo) "
  ...
  Future<List<SugerenciaBusquedaModel>> obtenerSugerencia() async {
    try {
      ...
      List<SugerenciaBusquedaModel> list = res.isNotEmpty ? res.map((c) => SugerenciaBusquedaModel.fromJson(c)).toList() : [];
      return list;
    } catch (e) {
